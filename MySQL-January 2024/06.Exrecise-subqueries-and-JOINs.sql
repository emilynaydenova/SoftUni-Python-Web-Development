...
-- 09.	Employee Manager -self-referncing
SELECT 
    e.employee_id,
    e.first_name,
    e.manager_id,
...
ORDER BY c.country_namecurrencies
LIMIT 5;

-- 15.	*Continents and Currencies
 
 WITH RankedCurrencies AS (
    SELECT
        cont.continent_code,
        c.currency_code,
        COUNT(*) AS currency_usage,
        RANK() OVER (PARTITION BY cont.continent_code ORDER BY COUNT(*) DESC) AS currency_rank
    FROM
        continents AS cont
    JOIN
        countries AS c ON c.continent_code = cont.continent_code
    GROUP BY
        cont.continent_code, c.currency_code
    HAVING
        currency_usage > 1
)
SELECT
